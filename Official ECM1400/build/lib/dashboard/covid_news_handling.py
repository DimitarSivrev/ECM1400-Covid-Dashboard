...
def news_API_request (covid_terms= news_api["covid_terms"] ):
    ...
    logging.info("Running news_API_request")
...
def update_news(removed_news_list):
    ...
    CDoesn't display news articles that the user has manually removed by comparing
    the items to the removed items list
    :parameter removed_news_list: List of removed news
    '''
    logging.info("Running update_news")
    try:
        news.clear()
        news_API_request()
...
def schedule_news_updates(update_interval,update_name, update_type = 'non-repeating'):
    '''
    ...
    '''
    logging.info("Running schedule_news_updates")
    #Using try and except to seek out any errors when processing the file
    try:
        ...
        if update_type == 'repeating':
            ...
            event_name= s.enter(86400,
            1,
            schedule_news_updates,
            kwargs={'update_interval' : 86400,
            'update_name': str(update_name),
            'update_type' : 'repeating'})
            events.append(event_name)
...
def removed_news_function(article):
    logging.info("Running removed_news_function")
    try:
        global removed_news_list
        removed_news_list.append(article)
    except:
        logging.error("Article wasn't added to the list of removed articles")
    else:
        logging.info("Article was added to the list of removed articles")
